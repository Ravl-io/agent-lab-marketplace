...
3. **Read every file in `data/sources/`.** All of them. The status updates are authoritative
   for milestone dates and status; the sync notes carry the reasons and the things nobody
   wrote up formally; the metrics file is the only source for counts.
