#!/usr/bin/env python3
"""tools/{{TOOL_NAME}}.py

{{TOOL_PURPOSE}}

Run it:

    {{TOOL_COMMAND}}
    python3 tools/{{TOOL_NAME}}.py --selftest

Two rules for a tool, and they are the reason this is a tool and not a skill:

  1. **Same input, same output.** No judgement, no summarising, no "it depends".
  2. **Structured output.** JSON, because the next thing to read it is a program, not a
     person. Print nothing else to stdout.

The argument handling and the JSON printing are done. What is missing is the part that
matters: getting the facts out, and proving you got them right.
"""

from __future__ import annotations

import argparse
import json
import os
import sys

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

import re


def add_arguments(ap: argparse.ArgumentParser) -> None:
    ap.add_argument("--dir", default="data/deliverables",
                    help="directory of submitted deliverables to scan")


def collect(args) -> dict:
    """Return the hard facts about the submitted deliverables.

    Your output must contain these keys, because your skill is going to rely on them:
    files, secrets, personal_data, endpoints, summary

    TODO — the scans. Every one of these is a regex over lines, not a judgement.

    Three decisions worth making deliberately rather than by accident:

      * **severity 1 first**: credentials and production personal data are S1 under MSA 6.1
        and 6.2, and they are exactly what a reviewer skims past late in the day. Which
        patterns catch them, and what do you report — the whole line, or a redacted excerpt?
      * **false positives**: `example.com` and friends are reserved for documentation. An
        address there is not production data. Where does that exception live, and how do you
        test that it works when the corpus has no example of it?
      * **absence as a finding**: "rate limits are not quantified anywhere" is a finding.
        How does your output say that a thing is missing, rather than just omitting it?
    """
    raise NotImplementedError("TODO: scan the deliverables")


def selftest() -> int:
    """A tool can be tested. That is most of why it is a tool.

    Add one check per fact you are certain of. When your tool later breaks, these tell you
    *what* broke, which a prompt never can. This is also your first eval — Module 4 is
    largely this idea, scaled up.
    """
    checks, failures = 0, []

    def expect(label: str, ok: bool) -> None:
        nonlocal checks
        checks += 1
        if not ok:
            failures.append(label)

    # TODO — at least three checks. One of them must be this one:
    # expect("same input, same output", collect(...) == collect(...))

    for f in failures:
        print(f"FAIL: {f}", file=sys.stderr)
    print(f"{checks - len(failures)}/{checks} checks passed")
    return 1 if failures else 0


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    add_arguments(ap)
    ap.add_argument("--selftest", action="store_true", help="check the tool against known facts")
    args = ap.parse_args()
    if args.selftest:
        return selftest()
    print(json.dumps(collect(args), indent=2))
    return 0


if __name__ == "__main__":
    sys.exit(main())
